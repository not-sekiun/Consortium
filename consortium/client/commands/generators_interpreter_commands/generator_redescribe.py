...
# Paired with `rename`: both act on an agent generator that already exists and are named
# for changing something that is already set. Staging the description of an agent
# generator that has yet to be created is `describe`, inside an agent template's context.
class GeneratorRedescribeCommand(BaseConnectedCommand):
    name = "redescribe"
