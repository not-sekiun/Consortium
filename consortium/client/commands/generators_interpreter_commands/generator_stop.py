...
class GeneratorStopCommand(BaseConnectedCommand):
    name = "stop"
    description = "Stop a running agent generator by its ID"
    epilog = format_argparse_epilog(
        """
        Examples:
          stop 123e4567-e89b-12d3-a456-426614174000
        """,
    )
    ...
    def configure_parser(self, parser: ArgumentParser) -> None:
        parser.add_argument(
            "agent_generator_id",
            help="ID of the agent generator to stop.",
            nargs=1,
        )
    ...
    async def run(self, context: ConnectedContext) -> InterpreterSignal:
        try:
            ...
            _ = await rest_api.stop_agent_generator_by_agent_generator_id(
                agent_generator_id=parsed_args.agent_generator_id[0],
            )
            print_success(
