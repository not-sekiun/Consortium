...
class StartGeneratorCommand(BaseCommand):
    ...
    description = (
        "Start a created agent generator with the currently set agent template options."
    )
    ...
    async def run_command(self, command_context: CommandContext) -> ReturnStatus:
        try:
            ...
            print_success(
                f'Started agent generator: "{agent_generator["name"]}" ({agent_generator["agent_generator_id"]})',
            )
        except SystemExit:
            pass
