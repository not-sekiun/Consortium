...
class AgentTemplateOptionCommand(BaseConnectedCommand):
    name = "option"
    description = "Manage the current agent template's options through its sub-commands"
    epilog = format_argparse_epilog(
        """
        Examples:
          option list
          option info remote_host
    ...
    def configure_parser(self, parser: ArgumentParser) -> None:
        ...
        parser_info = subparsers.add_parser(
            "info",
            help=(
                "Display information about a specific agent template option by its "
                "name."
            ),
            formatter_class=RawDescriptionHelpFormatter,
            epilog=format_argparse_epilog(
    ...
    def _handle_list_sub_command(
        ...
        agent_template_options = context.interpreter_context.agent_template["options"]
