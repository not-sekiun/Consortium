...
class ListenersInterpreter(ClientInterpreter):
    def __init__(self, client_connection: ClientConnection, listener_template_id: str):
        listeners_commands = [
        ...
        super().__init__(
            prompt=ANSI(
                export_rich_text_as_ansi(
                    f"[bold white]Consortium ([bold blue]Listeners: {listener_template_id}[bold white]) > ",
                ),
            ),
