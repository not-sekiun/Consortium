...
from consortium.framework.listeners.consortium.reverse_tcp.listener_type import (
    LISTENER_TYPE,
)
    ...
    async def on_listener_running(self) -> None:
        ...
        async def handle_agent(
            ...
            agent_handler = _AgentHandler(
                agents_manager=self.agents_manager, reader=reader, writer=writer
            )
            await agent_handler.run_agent_handler()

        await asyncio.start_server(
            client_connected_cb=handle_agent, host=local_host, port=local_port
        )
