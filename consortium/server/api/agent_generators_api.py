...
from consortium.server.server_exceptions import (
    ...
    AgentGeneratorStillRunningError,
    InternalServerError,
...
@router.delete(
    ...
    responses={
        200: {"model": SuccessResponseModel},
        404: {"model": AgentGeneratorNotFoundError().to_pydantic_model()},
        409: {"model": AgentGeneratorStillRunningError().to_pydantic_model()},
    },
)
def delete_listener_by_listener_id(
    ...
    try:
        ...
        if listener.status.state == ListenerState.RUNNING:
            raise AgentGeneratorStillRunningError
