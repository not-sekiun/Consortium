...
class ListenerTemplatesService:
    def __init__(self):
        ...
        listener_templates_root_path = pathlib.Path("consortium/server/listeners")
        visited_dir_paths = []
        for listener_project_folder_path in listener_templates_root_path.rglob("*"):
    ...
    def _load_listener_project_folder(
        ...
        # listener.py file and a listener_template.py file. The listener.py file must
        # contain a class called Listener that inherits from BaseListener. The
        # listener_template.py file must contain a class called ListenerTemplate that
        # inherits from BaseListenerTemplate. The folder must be located at
        # consortium/server/listeners with any arbitrary nested folder structure. On top
        # of that, the folder must be a valid python package reachable from the root of
        # the server. This means that the folder must contain __init__.py files in all
        # parent directories.
        files_in_directory = [
            file.name
        ...
        if (
            ...
            raise ValueError(
                f"Invalid listener project folder: {listener_project_folder_path}. The listener.py file is missing",
            )

        listener_project_folder_root_path = pathlib.Path("consortium/server/listeners")
        relative_path = listener_project_folder_path.relative_to(
            listener_project_folder_root_path,
        )
        module_name = ".".join(relative_path.parts)

        # check for valid python package structure
        try:
            listener_template_module = importlib.import_module(
                f"consortium.server.listeners.{module_name}.listener_template",
            )
            listener_module = importlib.import_module(
                f"consortium.server.listeners.{module_name}.listener",
            )
            listener_type_module = importlib.import_module(
                f"consortium.server.listeners.{module_name}.listener_type",
            )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid listener project folder: {listener_project_folder_path}. The listener.py file does not contain a class called Listener that inherits from BaseListener",
            )
        ...
        if not type(listener_type) == ListenerType:
            raise ValueError(
    ...
    def get_listener_template_by_listener_template_id(
        ...
        try:
            return self._listener_templates[listener_template_id]
        except KeyError:
            raise ValueError(
    ...
    def get_all_listener_templates(self) -> list[Type[BaseListenerTemplate]]:
        return list(self._listener_templates.values())
