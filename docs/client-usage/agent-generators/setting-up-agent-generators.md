...
| `describe <generator_id> <description>`               | Change an existing generator's description                       |
| `delete <generator_id>`                               | Delete a non-running generator                                   |

`rename` and `describe` act on a generator that already exists. To choose the name and
description a generator is *born* with, pass `-n/--name` and `-d/--description` to the
`create` command inside the template's context. See
