...
API, websockets are not covered by [OpenAPI](https://www.openapis.org/). This page is the only reference for
the events API.
...
3. Send action commands as JSON strings to subscribe, unsubscribe, or query subscriptions.
...
focuses on the standalone case, building a simple notification system from the ground up.
