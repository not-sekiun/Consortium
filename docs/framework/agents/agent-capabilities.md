...
## Using request_response_capability

`request_response_capability` is a factory that creates a `BaseAgentCapability` subclass
for the common case: send one task message, receive one reply. It is the right choice
for most commands.

```python
from consortium.framework.agents import request_response_capability
...
This produces a capability named `"info"` with the default behaviour: send the
`TaskLaunchMessageModel` as-is, wait for one `TaskOutputMessageModel` reply, and return
`Success` or `Failure` based on `reply.success`.

Add `options` to let operators configure the command before it is sent:
...
shell_capability = request_response_capability(
    ...
    options={
        ...
        SingleValueOption(
            name="timeout",
            description="Seconds to wait for output before timing out.",
            required=False,
            default_value=10,
            value_type=int,
...
Options are validated against operator input before `execute()` is called. The resolved
`arguments` dict is included in the `TaskLaunchMessageModel` sent to the agent.

### Customising with task_handler and result_handler

`task_handler` runs before the message is transmitted; `result_handler` runs after the
reply arrives. Both receive the agent, the message, and a `context` `SimpleNamespace`
that persists across the two calls:

```python
from datetime import datetime


def _ping_task_handler(agent, task_message, context):
    context.started_at = datetime.now()
    task_message.arguments.pop("timeout")  # server-side only, strip before sending
    return task_message


def _ping_result_handler(agent, result_message, context):
    delta = datetime.now() - context.started_at
    result_message.message = f"Latency: {delta.total_seconds():.3f}s"
    return result_message


ping_capability = request_response_capability(
    name="ping",
    description="Measure round-trip latency to the agent.",
    authors={"Your Name"},
    options={
        SingleValueOption(
            name="timeout",
    ...
    task_handler=_ping_task_handler,
    result_handler=_ping_result_handler,
)
```

`resolve_timeout` returns the per-invocation receive timeout in seconds. Without it the
static `timeout` parameter passed to the factory is used.
